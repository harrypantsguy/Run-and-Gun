...
namespace _Project.CodeBase.Editor
{
    public class CustomEditor<T> : OdinEditor where T : MonoBehaviour
    {
        ...
        protected void DrawPositionHandle(ref Vector2 targetPoint)
        {
            targetPoint = Handles.PositionHandle(position: targetPoint, rotation: Quaternion.identity);
        }
        
        protected void DrawCircleHandle(ref Vector2 targetPoint, ref float debugSize)
        {
            debugSize = Handles.RadiusHandle(rotation: Quaternion.identity, position: targetPoint, radius: debugSize);
            targetPoint = Handles.PositionHandle(position: targetPoint, rotation: Quaternion.identity);
        }
        ...
        {
            Vector2 angleDir = MathUtilities.AngleToDirection(angle: angle);
            ...
            lineTip = center + MathUtilities.AngleToDirection(angle: angle) * sliderDist;
        ...
        protected void DrawWireCube(Vector2 position, Vector2 size, float lineThickness)
        {
            Vector2 extents = size / 2f;
            Handles.DrawAAPolyLine(lineThickness, 
                position + extents, 
                position + new Vector2(extents.x, -extents.y),
                position - extents,
                position + new Vector2(-extents.x, extents.y),
                position + extents);
        }
